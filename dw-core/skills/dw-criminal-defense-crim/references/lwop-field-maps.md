...
| H-3 | Indictment Date | Date on Grand Jury Indictment / Bill of Information | The filing date printed on the indictment or bill of information document itself — not the offense date |
...
| H-12 | AGE AT TIME OF OFFENSE: | Calculate from client's DOB (booking/intake, rap sheet, police report) and offense date | Compute age by comparing DOB to offense date. If DOB unavailable, note approximate age from documents. |
...
| H-20 | INDICTMENT ATTACHED: | Case folder check | **Always mark Yes.** The indictment is present if we are populating this sheet. |
| H-21 | THEORY OF THE CASE — Initial Theory | Attorney notes / Criminal Defense Cover | **Attorney field.** Use red font with `[ATTORNEY]` placeholder. Leave blank if no notes. |
| H-22 | THEORY OF THE CASE — Trial Theory | Attorney notes | **Attorney field.** Use red font with `[ATTORNEY]` placeholder. Always leave blank. |
| H-23 | PRIOR CONVICTIONS: | Client's rap sheet / NCIC printout | Extract from rap sheet. Format: MM-DD-YYYY -- Offense Name. Include dispositions where available. |
| H-24 | WITNESS(ES): | Police Report → All statements → Transcripts | **Numbered list** (1, 2, 3…). Each entry: witness name + relationship to case (e.g., "1. Det. Smith (lead detective)"). |
| H-25 | WITNESS STATEMENT(S): | All witness statements | **Numbered list** (1, 2, 3…), one per witness. Each entry: witness name, who took statement, date/time, detailed summary. Bold names and quotes. Note inconsistencies. |
| H-26 | POLICE REPORT: | Incident/supplemental reports | Times, locations, officer actions, dispatch/arrival. |
| H-27 | DEFENSE(S): | Attorney notes / Case analysis | **Attorney field.** Use red font with `[ATTORNEY]` if no notes. |
| H-28 | POSSIBLE DEFENSE WITNESS(ES): | Attorney notes / Case analysis | **Attorney field.** Use red font with `[ATTORNEY]` if no notes. Alibi, character, expert witnesses. |
...
| H-53 | Does Defendant want to testify? | — | **Attorney field.** Always leave blank. Use red font with `[ATTORNEY]` placeholder. |
