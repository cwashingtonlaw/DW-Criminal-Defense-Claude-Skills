...
- `dw-neutral-inventory` (Report 0 — every catalog entry)
- `dw-theory-deconstructor` (Report 2a — every fact extraction)
- `dw-adversarial-stress-test` (every evidence citation)
- `dw-theory-to-workplan` (every task predicate)
- `dw-timeline-builder` (every timeline event)
- `dw-discovery-compliance-monitor` (every ledger entry)
