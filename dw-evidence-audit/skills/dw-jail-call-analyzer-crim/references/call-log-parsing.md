...
Vendor-by-vendor ingestion guide for jail-call discovery productions. Used by `dw-jail-call-analyzer-crim` Module A.
...
- Calls in log without audio (production deficiency — flag for `dw-discovery-compliance-monitor-crim`)
...
- From: at least the date of booking (request booking record from `dw-court-jail-tracker-crim` if not in hand)
...
- Cross-reference against `dw-witness-threat-matrix-crim` if built
...
- **Spanish-language calls.** Note in the Methodology section if the corpus includes non-English calls. Transcript pipeline must be configured for the appropriate language; route through `dw-transcript-pipeline-rev-crim` with the language code set explicitly.
