...
Used by `dw-jail-call-analyzer-crim` Module D. Provides the pattern catalog for witness tampering, witness intimidation, evidence tampering, and obstruction-of-justice exposure surfaced from recorded jail calls.
...
Severity: HIGH or CRITICAL. Triggers cross-feed to `dw-chain-of-custody-auditor-crim` if the evidence in question is on the State's exhibit list.
...
|------------------|-----------|---------|
| Any flag involving a named State's witness | `dw-witness-threat-matrix-crim` (Refresh Mode) | Updated Damage / Vulnerability scores; new impeachment hooks |
| Any relay person identified | `dw-defense-investigator-tasking-crim` | Investigator tasking to interview relay person and assess co-conspirator exposure |
| Any asset-disposal flag implicating exhibit-list evidence | `dw-chain-of-custody-auditor-crim` | Chain-of-custody audit for the affected exhibit |
| Any CRITICAL flag | Verbal counsel notification | Direct attorney call before document finalization |
| Any pattern affecting bond conditions | `dw-bond-and-release-motion-crim` | Bond-posture review for revocation exposure |
