...
description: >
  ...
  custodial interrogations — use dw-confession-interrogation-auditor-crim. Do NOT use for
  child forensic interviews — use dw-child-forensic-interview-auditor-crim.
---
...
## STEP 1A — Load Case Context from dw-case-brain-crim

Before analyzing any statements, load case context:

**Action:** Invoke dw-case-brain-crim to retrieve:
...
**Fallback:** If dw-case-brain-crim is unavailable or returns incomplete data, pause and ask the attorney:
...
- Analysis cards feed into **dw-cross-exam-architect-crim** for cross-examination outlines
- Credibility issues → **dw-brady-giglio-auditor-crim** (recommend running if witness has credibility red flags)
- Timeline data → **dw-timeline-builder-crim** (when available)
- Conflict matrix informs **dw-404b-opposition-crim**, **dw-suppression-motion-crim**
...
2. **Non-Custodial Only:** This skill handles non-custodial witness statements. For custodial interrogations → dw-confession-interrogation-auditor-crim. For child forensic interviews → dw-child-forensic-interview-auditor-crim
3. **Credibility Presentation:** Never draw ultimate conclusions — present indicators and let attorney decide
4. **Brady/Giglio Flags:** Flag potential Brady/Giglio material and recommend running dw-brady-giglio-auditor-crim if witness has bias, pending charges, or cooperation deal

---

**Integration:** Reads from dw-case-brain-crim (case context), case discovery, transcripts from dw-transcript-router-crim. Feeds into dw-cross-exam-architect-crim, dw-brady-giglio-auditor-crim, dw-timeline-builder-crim, Case Tables.xlsx (Witnesses sheet). Case Tables.xlsx is the centralized case data workbook maintained at the case folder root. The Witnesses sheet tracks all analyzed witnesses and their assessment summaries. Uses docx and xlsx skills for output.