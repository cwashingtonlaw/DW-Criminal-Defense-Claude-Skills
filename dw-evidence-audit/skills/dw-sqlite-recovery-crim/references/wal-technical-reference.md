...
**For use by `dw-sqlite-recovery-crim`. Internal reference. Treat as expert preparation material.**
...
- **Motion to Compel Production** — raw `.db`, `-wal`, `-shm` files, extraction logs, tool validation records. Produced via `dw-pretrial-motion-library-crim`.
- **Motion for Independent Examination** — defense expert re-extraction with WAL-aware tooling. Produced via `dw-pretrial-motion-library-crim`.
- **Daubert / Foret Challenge** — challenging the State's expert under La. C.E. Art. 702 when the methodology omits WAL analysis. Produced via `dw-expert-witness-evaluator-crim`.
- **Cross-examination outlines** — produced via `dw-cross-exam-architect-crim` from the seeds in Step 7 of the parent skill.
