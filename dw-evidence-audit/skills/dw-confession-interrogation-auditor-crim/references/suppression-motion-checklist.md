...
**Scope:** Confessions and statements only. For physical evidence suppression (Fourth Amendment search/seizure), route to `dw-suppression-motion-crim`.
...
|---|-----------|-----------------|
| 1 | Caption (court, parties, docket number, motion title) | Standard pleading; verify against `dw-shared-protocols-crim` template-selection-protocol results |
...
| Transcript of interrogation | `dw-transcript-router-crim` output | Citable substitute for recording |
