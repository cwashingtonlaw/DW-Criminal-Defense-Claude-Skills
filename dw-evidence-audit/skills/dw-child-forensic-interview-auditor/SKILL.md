...
Before detailed module analysis, identify which forensic interview protocol **should have been used** versus what was actually used. Six recognized protocols are commonly encountered: CornerHouse RATAC, NICHD, Cognitive Interview, Step-Wise (Yuille), Finding Words / ChildFirst, and the Ten-Step Investigative Interview. Each has its own age range, structural phases, compliance markers, and red flags.

A triage decision tree helps select which protocol *should have* been used given the child's age, the interviewer's role, and whether the interview was recorded — and a compact comparison matrix summarizes structural differences across all six.

**Reference:** Read `references/forensic-interview-protocols.md` for the full protocol descriptions, compliance markers, deviation red flags, the triage decision tree (Q1–Q4), and the Protocol Comparison Matrix.

---

## STEP 3 — MODULE A: Interviewer Qualification & Training Audit

Audit who conducted the interview. Areas covered: formal education, forensic-interview training hours and protocols, experience with the child's age range and the relevant allegation type, agency standards and supervision, and bias / contamination-control training (especially confirmation bias and source-monitoring training).

A red-flag table classifies common qualification gaps as CRITICAL or SIGNIFICANT (e.g., no formal training; obsolete training without recertification; interviewer not blind to allegations; interview not recorded).

**Reference:** Read `references/interviewer-and-environment-audit.md` (MODULE A section) for the full credentialing question set and the Module A red-flag table.

---

## STEP 4 — MODULE B: Interview Environment & Setup Audit

Audit the conditions under which the interview took place. Areas covered: physical environment (location, room setup, recording equipment), personnel presence in the room, interviewer's prior knowledge (and primed-suspicion contamination check), pre-interview communication contamination by parents / teachers / earlier interviewers, props and anatomical-tools presentation, and the child's observable comfort/coercion state.

**Reference:** Read `references/interviewer-and-environment-audit.md` (MODULE B section) for full environment-and-setup checklists, the "who was in the room" analysis, and standards for proper anatomical-tool use.

---

## STEP 5 — MODULE C: Rapport-Building & Ground Rules Audit

Audit how the interviewer opened the interview. Areas covered: presence and quality of an explicit rapport phase, whether the five core ground rules were explicitly established ("tell the truth," "say 'I don't know'," "tell me if I get something wrong," "I don't know what happened," "we're not guessing"), whether a practice narrative was used, and how the interviewer transitioned to the substantive topic.

A red-flag table identifies CRITICAL openings such as launching with "Did he touch you?" or modeling abuse with dolls before any narrative.

**Reference:** Read `references/interviewer-and-environment-audit.md` (MODULE C section) for the rapport-phase checklist, ground-rules framework, practice-narrative test, and Module C red-flag table.

---

## STEP 6 — MODULE D: Questioning Technique Audit (CORE MODULE)
...
## STEP 13 — Severity Classification & Audit Output
...
- Read `references/legal-authority-and-research.md` for the Louisiana, federal, and research authorities feeding the Legal Analysis section.

---

## Output Format & Path

Save the formal audit deliverable to:

```
{{CASE_ROOT}}/01 - Trial Notebook/09 - Case Analysis/Cowork Analysis/Forensic Interview Audit/
```

Filename convention: `Forensic_Interview_Audit_[ClientName]_[YYYY-MM-DD].docx`

Apply attorney work-product marking per `dw-shared-protocols/references/attorney-work-product-marking.md`. Include the case caption, audit date, and overall severity rating in the header.
...
This audit feeds directly into cross-examination strategy. For each finding, dw-cross-exam-architect can generate:
...
If the attorney wants to run a parallel audit in NotebookLM (e.g., for Red Team simulation, behavioral baseline check, coaching scan, escalation tracker, or generic forensic-interview audit), copy-paste prompts are available.

**Reference:** Read `references/notebooklm-audit-prompts.md` for Prompts A–E and the Golden Rule follow-up.
