...
**First, gather the case essentials from dw-case-brain-crim (or fallback chain if unavailable):**

### Primary Path: Invoke dw-case-brain-crim

Invoke `dw-case-brain-crim` and request:
...
### Fallback Chain (if dw-case-brain-crim unavailable)

**Fallback 1:** Invoke `dw-case-dashboard-crim` for:
...
- Discovery issues not yet resolved (see integration note below for dw-discovery-compliance-monitor-crim)
...
If dw-discovery-compliance-monitor-crim confirms outstanding discovery, the client CAN be told:
...
If dw-plea-negotiation-analyzer-crim is active on the case, do NOT reference plea discussions, but DO know they're happening (to avoid suggesting false expectations about trial).

**EXCEPTION — Pre-Sentencing Communications:**
If client is awaiting sentencing, dw-sentencing-mitigation-specialist-crim may inform this communication. Acknowledge mitigation preparation without disclosing strategy.
...
- **Use as Fallback 1:** If dw-case-brain-crim unavailable, pull court dates and case phase from dashboard
...
### dw-post-conviction-relief-crim
