...
### Source from dw-appellate-error-monitor-crim
...
1. **One assignment per preserved issue.** If `dw-appellate-error-monitor-crim` Module H ranked five preserved errors as Tier 1 or Tier 2, the brief should contain five assignments of error (plus errors patent, if any).
...
Source from `dw-appellate-error-monitor-crim` Module A (objection log) — the exact transcript page/line where the defense objected with specific grounds, and the trial court's ruling. Examples:
...
For Louisiana cases, use the Louisiana parallel-citation format from `dw-shared-protocols-crim/references/louisiana-citation-style.md`.
...
Source from `dw-appellate-error-monitor-crim` Module F — the harmless-error pre-assessment. The Module F output flags whether each error is structural, constitutional-Chapman, or non-constitutional-Art. 921, and provides the analytical skeleton.
