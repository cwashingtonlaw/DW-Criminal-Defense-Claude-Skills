...
- Read `../../../dw-core/skills/dw-shared-protocols-crim/references/template-selection-protocol.md` — DEVONthink template search protocol

### Step 2.5 -- Load Shared Protocols

Before drafting, read `dw-shared-protocols-crim/SKILL.md` and load the references listed for "State criminal motion (14th JDC Calcasieu)". If the active case is in a different parish, load the references for the corresponding parish row instead. If no row exists for the parish, load `caption-criminal-fill-in.md` and prompt the attorney for the court-specific values.
...
- Caption, signature block, and certificate of service per shared protocols (`dw-shared-protocols-crim`)
- Citation style per `dw-shared-protocols-crim/references/louisiana-citation-style.md`
...
- **Work product marking:** apply per `dw-shared-protocols-crim/references/attorney-work-product-marking.md` ONLY for INTERNAL DRAFTS produced for attorney review. The FILED VERSION of the sentencing memorandum receives NO work product marking.
...
| Skill | Purpose |
|-------|---------|
| **dw-shared-protocols-crim** | Caption, signature block, certificate of service, work product marking, citation style, output path formula — load before drafting any pleading |
| **dw-criminal-defense-crim** | Master case management workflow (Phases 0-3) |
| **dw-sex-offense-specialist-crim** | Sex offense-specific sentencing considerations |
| **dw-expert-witness-evaluator-crim** | Challenge prosecution experts at sentencing |
| **dw-child-forensic-interview-auditor-crim** | Juvenile sentencing cases involving forensic interview evidence |
| **dw-404b-opposition-crim** | Prior bad acts evidence at sentencing |
| **dw-discovery-compliance-monitor-crim** | Ensure all discovery relevant to sentencing has been produced |
| **dw-voir-dire-assistant-crim** | Penalty-phase jury selection in capital and LWOP cases |
