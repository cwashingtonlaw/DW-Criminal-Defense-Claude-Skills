...
Used by `dw-jail-call-analyzer` Module B. Provides the canonical admission categories, the damage-severity rubric, and worked examples to anchor the scoring.
...
Cross-feed every Module B Category-5 flag to `dw-404b-opposition` so the defense can prepare a 404(B) opposition before the State files notice.
...
Cross-reference every Tier 1 call against the defense theory pulled from `dw-case-brain` in Step 1. Each contradiction must be flagged with:
