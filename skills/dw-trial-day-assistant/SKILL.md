...
description: >
  ...
  longer-form end-of-day memo. Feeds dw-appellate-error-monitor (objection log),
  dw-cross-exam-architect (witness scorecard → tomorrow's cross), and
  dw-trial-notebook-builder (daily roll-up). Do NOT use for pre-trial prep
  (dw-cross-exam-architect, dw-jury-instructions-builder) or post-verdict appellate
  audits (dw-appellate-error-monitor in full audit mode).
---
...
Before producing any deliverable, read `dw-shared-protocols/SKILL.md` and load:

1. `dw-shared-protocols/references/attorney-work-product-marking.md` — every output is internal work product
2. `dw-shared-protocols/references/output-path-formula.md` — for output paths anchored on `CASE_ROOT`
...
7. **Critical witnesses today** — anyone flagged CRITICAL in `dw-witness-threat-matrix`
...
1. WITNESSES UP TODAY (in order)
   1. [Witness] — State direct, est. [N] hr — CRITICAL/HIGH/MED
      ...
      Threat matrix priority: [from dw-witness-threat-matrix]
   2. [Witness] — ...
...
**Purpose:** Rolling log of every objection. **CRITICAL: This module is the upstream feed for `dw-appellate-error-monitor`. Match its input data shape exactly so the appellate audit can ingest this log without re-keying.**

**Trigger phrases:** "log this objection," "objection log," "log obj," "log a missed objection," "MO log"

### Objection Log Row Format (matches dw-appellate-error-monitor MODULE A schema)
...
Run a parallel log for objections counsel intended but did not make. Schema matches `dw-appellate-error-monitor` MODULE B:
...
At end of trial, the running master is `Objection Log - MASTER.md` and feeds directly into `dw-appellate-error-monitor` MODULE A.

### Handoff to dw-appellate-error-monitor
...
**Purpose:** Half-page debrief per witness. Feeds `dw-cross-exam-architect` for tomorrow's cross prep when a defense-case witness is up next, or for redirect/recross planning if the State's witness is still pending.
...
- [bullet] — escalate to dw-cross-exam-architect for next-day prep
...
### Handoff to dw-cross-exam-architect
...
10. OVERNIGHT TASKS
    - Research: [question + skill to invoke — e.g., dw-case-law-researcher]
    - Witness calls: [name + purpose]
    - Exhibit prep: [item]
    - Motion drafts: [motion + skill — e.g., dw-pretrial-motion-library or dw-suppression-motion]
...
- `dw-cross-exam-architect` — for the tomorrow-witness cross prep
- `dw-pretrial-motion-library` / `dw-suppression-motion` — for any motions to draft tonight
- `dw-jury-instructions-builder` — if today raised a new instruction issue
- `dw-trial-notebook-builder` — at end of trial, all daily memos roll up into the trial notebook
...
- All issues → roll into `dw-appellate-error-monitor` post-trial as either preserved errors or missed objections
- Limiting / curative instruction needs → feed back to `dw-jury-instructions-builder` for any final-instruction adjustment
...
Output format is **always Markdown** (`.md`) — fast to read, easy to grep, easy to print. No `.docx` mid-trial; no time. End-of-trial roll-up to `.docx` happens via `dw-trial-notebook-builder`.

Apply the work product marking header per `dw-shared-protocols/references/attorney-work-product-marking.md` to every file.
...
- **Do NOT do pre-trial cross-exam preparation.** That's `dw-cross-exam-architect`. Trial Day Assistant produces tomorrow-prep handoffs but does not draft chapter-based cross outlines.
- **Do NOT do full appellate audits.** That's `dw-appellate-error-monitor` post-verdict. Trial Day Assistant feeds the appellate monitor with raw objection-log data; it does not assess preservation status with finality.
- **Do NOT do witness threat scoring.** That's `dw-witness-threat-matrix`. Trial Day Assistant references existing threat-matrix priority but does not compute new Damage / Vulnerability scores.
- **Do NOT draft jury instructions.** That's `dw-jury-instructions-builder`. Trial Day Assistant flags instruction needs (limiting, curative); the instruction-builder skill drafts them.
...
- **Field names match downstream consumers.** Do not rename objection-log fields, missed-objection fields, or exhibit-tracker fields without coordinating with `dw-appellate-error-monitor` and `dw-trial-notebook-builder`.
...
- **`dw-appellate-error-monitor`** — Module B Objection Log and Missed Objection Sub-Log feed directly into Appellate Error Monitor's MODULE A and MODULE B. Field schemas are deliberately identical.
- **`dw-cross-exam-architect`** — Module C Witness Scorecards feed the "WHAT CROSS STILL NEEDS" bullets into next-day cross-outline drafting.
- **`dw-trial-notebook-builder`** — All daily Module A-G outputs roll up into the final trial notebook at end of trial. The end-of-day memos (Module F) become the day-by-day record.
- **`dw-jury-focus-group`** — Module E Juror Observation Log can feed juror-research follow-up if a particular juror's reaction pattern raises a question worth researching.

**Pairs with:**

- **`dw-jury-instructions-builder`** — Module G (limiting / curative instruction flags) feeds back to instruction-builder for any final-charge adjustments.
- **`dw-witness-threat-matrix`** — Module C scorecards can refresh the threat matrix at end of week (Post-Cross Refresh Mode in the threat-matrix skill).
- **`dw-pretrial-motion-library`** / **`dw-suppression-motion`** — Module F overnight tasks may include motion drafts to be generated by these skills.
- **`dw-case-law-researcher`** — Module F overnight research tasks routed to this skill.

**Upstream inputs:**

- `dw-witness-threat-matrix` — for today's witness CRITICAL/HIGH priority labels in Module A
- `dw-cross-exam-architect` — existing cross outlines to reference in Module A and recall during Module C scorecards
- `dw-case-brain` — for case theme, defense theory, case caption, attorney info
...
- v1.0.0 — initial build. Seven modules (A-G). Field schemas aligned with `dw-appellate-error-monitor` MODULE A / B for direct handoff. Output path: `{{CASE_ROOT}}/01 - Trial Notebook/04 - Trial Day/Day [N]/`.
