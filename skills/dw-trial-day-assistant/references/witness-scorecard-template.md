...
Length target: half a page per witness. The exception is a CRITICAL witness (per `dw-witness-threat-matrix`) where a full page is acceptable.
...
- [unfinished business] — escalate to dw-cross-exam-architect
...
------------------------------------------------------------
[For dw-witness-threat-matrix Post-Cross Refresh Mode]
...
CRITICAL scorecards may run a full page. They feed `dw-cross-exam-architect` for any redirect / recall and `dw-witness-threat-matrix` Post-Cross Refresh.
...
- **`dw-cross-exam-architect`** — section 6 ("WHAT CROSS STILL NEEDS") seeds the next-day cross outline for this witness or related witnesses.
- **`dw-witness-threat-matrix`** — section 10 ("THREAT-MATRIX REFRESH SIGNAL") feeds Post-Cross Refresh Mode after multiple scorecards accumulate.
- **`dw-appellate-error-monitor`** — section 11 ("APPELLATE FLAGS") cross-references Module B objection log.
- **`dw-trial-notebook-builder`** — at end of trial, all scorecards roll into the Trial Notebook witness section.
...
- Not a substitute for the cross outline (`dw-cross-exam-architect` produces the outline; the scorecard captures what actually happened).
...
*Apply work product marking per `dw-shared-protocols/references/attorney-work-product-marking.md`.*
