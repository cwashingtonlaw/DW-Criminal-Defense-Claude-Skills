...
Standardized output path convention for all D&W file-writing skills. Anchored on `CASE_ROOT` (exposed as a YAML variable in `dw-case-brain` v3.3+).
...
`CASE_ROOT` is resolved by `dw-case-brain` based on the case parish/jurisdiction:
...
The completed trial notebook is assembled in place using the existing `01 - Trial Notebook/` structure — `dw-trial-notebook-builder` indexes the curated tabs rather than creating a new one.
