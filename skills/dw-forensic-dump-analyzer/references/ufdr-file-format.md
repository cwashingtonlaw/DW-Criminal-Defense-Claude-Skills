...
**CRITICAL:** If the UFDR contains raw SQLite databases, hand off to **dw-sqlite-recovery** for WAL analysis before proceeding — WAL data may not survive repeated file access. Write-Ahead Log files (*.db-wal) contain uncommitted transactions that can recover deleted records.