...
*This reference is loaded by the dw-forensic-dump-analyzer skill during Step 4 (Cross-Reference Mode). It is only relevant when the attorney provides case documents alongside the phone data. If no case documents are provided, the skill operates in standalone mode and skips this reference.*