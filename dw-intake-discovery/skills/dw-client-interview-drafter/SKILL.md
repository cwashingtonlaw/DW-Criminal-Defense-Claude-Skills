...
description: >
  ...
  dw-client-communication-drafter), witness interviews (use dw-witness-statement-analyzer),
  or investigator tasking (use dw-defense-investigator-tasking).
---
...
1. `dw-shared-protocols/references/attorney-work-product-marking.md` — apply work product header
2. `dw-shared-protocols/references/output-path-formula.md` — anchor output on `CASE_ROOT`
...
1. **`dw-case-dashboard`** — for court schedule and phase
...
5. Recommended next routing (e.g., "After this interview, route to dw-suppression-motion if Miranda issues confirmed")
...
| Upstream | Provides |
|----------|----------|
| `dw-case-brain` | Primary case context |
| `dw-case-dashboard` | Court schedule, phase |
| `dw-criminal-defense` | Phase-aware context for which mode fits |

| Downstream (post-interview routing) | Triggered by |
|--------------------------------------|--------------|
| `dw-suppression-motion` | Miranda / search issues identified during interview |
| `dw-bond-and-release-motion` | Bond mitigators captured during interview |
| `dw-defense-investigator-tasking` | Witness leads / scene questions identified |
| `dw-sentencing-mitigation-specialist` | Mitigation seeds captured |
| `dw-habitual-offender-auditor` | Prior record verification raises Boykin questions |
| `dw-plea-negotiation-analyzer` | Pre-plea follow-up sheet feeds plea analysis |
| `dw-discovery-compliance-monitor` | Discovery gaps identified during post-discovery follow-up |
| `dw-issue-code-tracker` | Issue codes opened / closed based on client answers |
| `dw-case-brain` | Session delta saved after interview |
