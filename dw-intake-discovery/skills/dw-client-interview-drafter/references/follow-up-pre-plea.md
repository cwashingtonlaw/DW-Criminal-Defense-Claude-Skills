...
Run by `dw-plea-negotiation-analyzer` output. Confirm client understands:
...
Per `dw-plea-negotiation-analyzer`:
...
- If accepting: prep Boykin colloquy support, restitution backup, allocution outline → `dw-pretrial-motion-library`
- If rejecting: file rejection on record, lock down trial date, refresh trial-prep timeline → `dw-trial-notebook-builder`
- If sentencing is open: prep mitigation packet → `dw-sentencing-mitigation-specialist`
- Update Case Brain with decision and reasons → `dw-case-brain`
- Document advisement for appellate / PCR record → `dw-appellate-error-monitor`
