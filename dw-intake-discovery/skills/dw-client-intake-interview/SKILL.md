...
description: >
  ...
  skills. Do NOT use for case file setup once retained — that is dw-criminal-defense
  Phase 1. Do NOT use for ongoing client communications — use dw-client-communication-drafter.
---
...
Intake is **upstream of every other skill** in the firm's collection. Its outputs seed `dw-criminal-defense` Phase 1, `dw-bond-and-release-motion`, `dw-defense-investigator-tasking`, `dw-case-brain`, and `dw-billing-narrative-generator`.
...
Before producing any deliverable, read `dw-shared-protocols/SKILL.md` and load these references:

1. `dw-shared-protocols/references/attorney-work-product-marking.md` — every intake deliverable is internal work product; mark all output headers
2. `dw-shared-protocols/references/output-path-formula.md` — for `CASE_ROOT` resolution and path conventions
3. `dw-shared-protocols/references/signature-block.md` — for the engagement letter signature block
...
|---|---|
| Drug offense (possession, distribution, manufacture, conspiracy) | `dw-drug-offense-specialist` |
| DWI / DUI / OWI | `dw-dwi-specialist` |
| Sex offense (any La. R.S. 14:42 series, 14:43 series, 14:80 series, 14:81 series) | `dw-sex-offense-specialist` |
| Violent crime (homicide, attempted homicide, armed robbery, aggravated battery, kidnapping) | `dw-violent-crime-specialist` |
| Firearm offense (felon-in-possession, illegal carry, convicted-felon firearm enhancements) | `dw-firearms-specialist` |
| Domestic violence / IPV | `dw-violent-crime-specialist` (with intake-question-bank-by-charge-type domestic violence module) |
...
**Route:** Pass the bond facts directly to `dw-bond-and-release-motion`. If the client is in custody and has not had a bail hearing or has an excessive bond, this is urgent — the bond motion should be drafted within 48 hours of retention.
...
Cowork produces the jail-call-hygiene client letter (templates from `dw-jail-call-analyzer`) covering:
...
The intake conversation produces the **seed input** for `dw-defense-investigator-tasking`. The seed is not the full investigation plan — it is the raw lead list. The investigator tasking skill turns it into prioritized assignments.
...
**Route:** Pass the seed to `dw-defense-investigator-tasking` once retention is final. The investigator tasking skill produces the task list, witness questionnaires, scene checklists, and records-request packets.

---

## MODULE F — Intake Memo + Scope of Representation

The intake memo is the single document that captures everything Modules A through E produced. It is the foundation document for `dw-case-brain` and is referenced by every downstream skill.
...
VI.   IMMEDIATE-ACTION ITEMS (Module D output)
      [Bond posture and routing to dw-bond-and-release-motion]
...
VII.  INVESTIGATION SEED (Module E output)
      ...
      Routed to dw-defense-investigator-tasking.
...
- [ ] Downstream skills queued: `dw-bond-and-release-motion` (if applicable), `dw-defense-investigator-tasking`, charge-type specialist, `dw-case-brain` (initial entry), `dw-billing-narrative-generator` (engagement scope)

If any gate item is incomplete, the intake is incomplete. Do not let the case advance to `dw-criminal-defense` Phase 1 setup until intake closes.
...
9. **Do not give a definitive plea or sentencing prediction at intake.** The client wants to know the answer; intake is not the moment for it. Give procedural information and exposure ranges; defer plea analysis to `dw-plea-negotiation-analyzer` once discovery is in.

10. **Mirandized client statements before retention.** If the client made statements to law enforcement, capture the circumstances precisely (time, place, who was present, recorded vs. not, Mirandized vs. not, invoked vs. not, signed any waiver). These are foundational for any future suppression motion. Route to `dw-suppression-motion` and `dw-confession-interrogation-auditor` post-intake.
...
|---|---|
| `dw-criminal-defense` | Intake memo precedes Phase 1 setup. The intake memo and engagement scope are inputs to Phase 1 Step 1 (Folder Setup) and Phase 1 Step 3 (Case Profile Section 1 Identification, Section 2 Charges, Section 3 Bail). |
| `dw-bond-and-release-motion` | Module D (Bond posture) is the direct input. If the client is in custody at intake, draft the bond motion within 48 hours. |
| `dw-defense-investigator-tasking` | Module E (Investigation Seed) is the direct input. The investigator tasking skill turns the seed into prioritized assignments. |
| `dw-case-brain` | Intake memo is the initial Case Brain entry. CASE_ROOT, defendant identification, charges, bond status, and key dates all populate from the memo. |
| `dw-billing-narrative-generator` | Engagement scope (Module F) sets fee structure and scope; downstream billing must align. |
| `dw-jail-call-analyzer` | Module D.4 (Jail call hygiene letter) uses templates from `dw-jail-call-analyzer`. Once the client makes calls, the analyzer audits them. |
| `dw-suppression-motion` | If client made law-enforcement statements, route facts post-intake. |
| `dw-confession-interrogation-auditor` | If the client gave a custodial statement, route post-intake for full audit. |
| `dw-social-media-auditor` | After lockdown (Module D.3), the auditor reviews captured social media for Brady, impeachment, and defense use. |
| `dw-drug-offense-specialist` / `dw-dwi-specialist` / `dw-sex-offense-specialist` / `dw-violent-crime-specialist` / `dw-firearms-specialist` | Charge-type dispatcher routes per Module B. The specialist runs on the charge category once retention is final. |
| `dw-plea-negotiation-analyzer` | Deferred — never run at intake. Run after discovery is in. |
| `dw-sentencing-mitigation-specialist` | Tier 3 contextual data (employment, family, military, mental health) seeds mitigation. Run later. |
| `dw-shared-protocols` | Loaded at Step 0.5 for marking, output paths, signature block. |
...
*This skill is part of the Daniels & Washington Cowork criminal defense toolkit. Version 1.0 establishes the first-contact intake protocol upstream of dw-criminal-defense Phase 1. Integrates with dw-shared-protocols (marking, output paths), dw-bond-and-release-motion (Module D bond posture), dw-defense-investigator-tasking (Module E seed), dw-case-brain (initial entry), dw-billing-narrative-generator (engagement scope), dw-jail-call-analyzer (hygiene letter), and the charge-type specialist family (Module B dispatcher).*
