...
**Auditor Route:** `dw-crime-scene-auditor-crim` + `dw-suppression-motion-crim` (constitutional scan for 4th Amendment seizure/search issues)
...
**Auditor Route:** `dw-mobile-forensic-auditor-crim` → `dw-forensic-dump-analyzer-crim` (for detailed deleted data / file system analysis)

**Secondary Route:** `dw-cell-site-geolocation-auditor-crim` (if location data extracted)
...
**Auditor Route:** `dw-video-evidence-auditor-crim`

**Secondary Route:** `dw-suppression-motion-crim` (if video shows constitutional violations)
...
**Auditor Route:** `dw-transcript-router-crim` (parish-based dispatch to `dw-transcript-pipeline-calcasieu-crim` or `dw-transcript-pipeline-rev-crim`) → `dw-confession-interrogation-auditor-crim` (for interrogation analysis and suppression issues)
...
**Auditor Route:** `dw-eyewitness-identification-auditor-crim`
...
**Auditor Route:** `dw-crime-scene-auditor-crim` (for lab methodology audit) + `dw-chain-of-custody-auditor-crim` (for evidence handling from collection through lab)
...
**Auditor Route:** `dw-witness-statement-analyzer-crim` → `dw-cross-exam-architect-crim` + `dw-brady-giglio-auditor-crim`

**Processing Note:** Run dw-witness-statement-analyzer-crim first to produce Witness Analysis Cards (key facts, inconsistencies, credibility indicators), which then feed into dw-cross-exam-architect-crim for cross-examination outline building.
...
**Auditor Route:** `dw-cell-site-geolocation-auditor-crim`

**Secondary Route:** `dw-brady-giglio-auditor-crim` (to flag if location data contradicts prosecution timeline)
...
**Auditor Route:** `dw-social-media-auditor-crim`

**Secondary Route:** `dw-brady-giglio-auditor-crim` (if social media shows inconsistency with prosecution narrative)
...
**Auditor Route:** `dw-suppression-motion-crim` (warrant audit mode: probable cause adequacy, particularity, execution compliance)
...
**Auditor Route:** `dw-child-forensic-interview-auditor-crim` (for interview methodology and suggestiveness analysis)
...
**Auditor Route:** `dw-expert-witness-evaluator-crim` (to assess expert reliability, bias, and cross-examination vulnerabilities)
...
**Auditor Route:** `dw-habitual-offender-auditor-crim` (to assess habitual offender exposure and prior conviction admissibility)
...
**Auditor Route:** `dw-brady-giglio-auditor-crim` (to flag undisclosed cooperation deals and Giglio impeachment material)
...
**Auditor Route:** `dw-sex-offense-specialist-crim` (comprehensive sex offense analysis including SANE audit, DNA mixture interpretation, rape shield, delayed disclosure research)

**Secondary Route:** `dw-chain-of-custody-auditor-crim` (for kit handling from collection through lab) + `dw-child-forensic-interview-auditor-crim` (if victim is a minor and forensic interview also in discovery)
...
**Auditor Route:** `dw-sqlite-recovery-crim` (WAL sequencing analysis, deleted data recovery audit, forensic standard-of-care assessment)

**Note:** These files are typically produced alongside a phone extraction (Category B). If the extraction report is also present, route the extraction report to `dw-mobile-forensic-auditor-crim` first, then raw database files to `dw-sqlite-recovery-crim`. The SQLite auditor evaluates what the extraction tool missed.
...
Any file classified under Categories A (Police Reports), C (Video Evidence), D (Audio Recordings), I (Cell Tower Records), or B (Phone Extractions) should ALSO be routed to `dw-timeline-builder-crim` as a secondary auditor.

**Auditor Route (Secondary):** `dw-timeline-builder-crim`

**Purpose:** After primary auditors process these files, dw-timeline-builder-crim aggregates all extracted timestamps into the master case timeline with conflict detection and source reliability scoring.

**Processing Note:** Run dw-timeline-builder-crim AFTER Priority 2 forensic audits complete, using all timestamped evidence to assemble the unified chronology. This runs parallel to Priority 3 witness audits.
