...
Apply caption, signature block, certificate of service, notice of hearing, proposed order, formatting, and filename conventions per shared protocols (see Step 0.5 — `dw-shared-protocols-crim`).
...
**Save location:** Use the output path formula from `dw-shared-protocols-crim/references/output-path-formula.md`. Filed motions go to `{{CASE_ROOT}}/02 - Pretrial Notebook/01 - Pleadings/`.
...
- Suppression issues → hand off to `dw-suppression-motion-crim`
- 404(b) issues → hand off to `dw-404b-opposition-crim`
