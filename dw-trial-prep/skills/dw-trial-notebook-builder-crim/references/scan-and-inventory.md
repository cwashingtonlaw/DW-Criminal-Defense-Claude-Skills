...
| Tab 3 | `01 - Trial Notebook/03 - Witnesses/` | Cross-exam outlines from `dw-cross-exam-architect-crim` (.docx only — that skill no longer produces a source catalog or combined sources PDF; index any that survive from older builds but never flag their absence as a gap), Direct-exam outlines from `dw-direct-exam-architect-crim` (.docx only — same as cross; index any catalogs surviving from older builds but never flag their absence as a gap), impeachment worksheets, witness battle cards, witness dossiers. Check both `Prosecution Witnesses/` and `Defense Witnesses/` subfolders |
