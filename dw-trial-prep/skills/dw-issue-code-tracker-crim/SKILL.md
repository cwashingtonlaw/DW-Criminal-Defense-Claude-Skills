...
**Capstone Aggregator:** No — feeds into dw-case-dashboard-crim and dw-trial-notebook-builder-crim
...
Read `../../../dw-core/skills/dw-shared-protocols-crim/SKILL.md` for:
...
**This skill does NOT:**
- Auto-invoke specialist skills (the attorney decides when to run `dw-suppression-motion-crim`, etc.)
- Replace `dw-case-dashboard-crim` (which shows deliverable status, not issue status)
- Replace `dw-case-brain-crim` (which handles session persistence)
...
- **Reads from:** `dw-case-brain-crim` for CASE_ROOT, `dw-criminal-defense-crim` for case type
- **Writes to:** Obsidian Case Brain (`Case-Brain.md`), Case Tables.xlsx (`Issue Codes` sheet), Deliverables folder
- **Referenced by:** `dw-case-dashboard-crim` (pulls Open count for status display), `dw-trial-notebook-builder-crim` (pulls full ledger for trial readiness check)
- **Does NOT trigger:** Specialist skills. The attorney decides when to invoke `dw-suppression-motion-crim`, `dw-eyewitness-identification-auditor-crim`, etc. The `Linked Skill` column is a reference map only.
...
All deliverables follow `dw-data-contracts-crim`:
