...
- **dw-pretrial-motion-library-crim** -- for motions in limine that affect which
  instructions will be needed (e.g., motion to exclude 404(B) evidence
  eliminates need for limiting instruction if granted)
- **dw-criminal-defense-crim** -- for identifying defense theories that drive
  affirmative defense instructions
- **dw-appellate-error-monitor-crim** -- for assessing which instruction errors are
  most likely to succeed on appeal and ensuring proper preservation
- **dw-voir-dire-assistant-crim** -- for coordinating jury instruction themes
  with voir dire questioning strategy
- **dw-trial-notebook-builder-crim** -- for sequencing instruction preparation within
