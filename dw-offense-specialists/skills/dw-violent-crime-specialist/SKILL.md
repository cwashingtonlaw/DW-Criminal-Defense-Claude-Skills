...
description: >
  ...
  "Article 814," or "death-eligible." Do NOT use for sex offenses (use dw-sex-offense-specialist),
  drug offenses (use dw-drug-offense-specialist), DWI (use dw-dwi-specialist), or standalone gun
  charges (use dw-firearms-specialist) — but DO use this skill when a violent crime is the lead
...
1. `dw-shared-protocols/references/attorney-work-product-marking.md` — apply work product marking to all internal deliverable headers.
...
Use `dw-case-brain` to pull the structured case context. The minimum dataset for this skill is:
...
- Co-defendant status and any cooperator agreements (route to `dw-brady-giglio-auditor`).
- Crime-scene reconstruction status (route to `dw-crime-scene-auditor`).
- Pretrial motion status (Prieur, severance, motions in limine — route to `dw-pretrial-motion-library`).
- Existing defense theory drafts (route from `dw-case-brain` and any prior memos).
...
Every factual claim made about the case in any deliverable produced by this skill must cite the exact discovery item that supports it. The format is the same as in `dw-drug-offense-specialist` and other charge specialists:
...
- **Identity.** Was the defendant the offender? Eyewitness ID under *State v. Hunt* / *Manson v. Brathwaite* reliability factors — route to `dw-eyewitness-identification-auditor`. Surveillance video clarity. DNA / fingerprint chain. Co-defendant blame-shifting and *Bruton* problems.
...
- **Defective charging instrument.** Bill of information ambiguities, multiplicity, duplicity — route to `dw-pretrial-motion-library`.
...
Route to `dw-pretrial-motion-library` for templates; route to `dw-jury-instructions-builder` for the charge package.
...
If a necessity theory is being raised, document the elements with discovery citations and route to `dw-pretrial-motion-library` for any motion in limine to admit the supporting evidence.
...
- Mitigation investigation begins immediately — route to `dw-sentencing-mitigation-specialist`.
...
LWOP cases compress all of the trial-defense effort into the verdict-form architecture. There is no sentencing mitigation that softens the LWOP outcome — the only way out is acquittal, manslaughter conversion, or charge reduction by plea. Route to `dw-plea-negotiation-analyzer` for plea modeling.
...
Mitigation investigation for the Art. 878.1 hearing requires neuropsychological evaluation, school records, family-history mitigation, brain-development science. Route to `dw-sentencing-mitigation-specialist`.
...
Workflow:
1. Pull all certified conviction documents (route to `dw-habitual-offender-auditor` for the audit).
...
7. Feed the model into `dw-plea-negotiation-analyzer` for plea-versus-trial decision.
...
Every violent-crime case should generate the following motion package, calibrated to the facts. Route to `dw-pretrial-motion-library` for templates.
...
- **Motion to suppress identification.** *Manson v. Brathwaite* / *State v. Hunt* reliability factors. Route to `dw-eyewitness-identification-auditor`.
...
- **Brady / Giglio demand.** All impeachment material on State witnesses, including informant deals, prior false statements. Route to `dw-brady-giglio-auditor`.
...
- **Motion for bond reduction / release on conditions.** Route to `dw-bond-and-release-motion`.
...
- Cell phone forensic dumps (defendant's, victim's if available). Route to `dw-mobile-forensic-auditor` and `dw-forensic-dump-analyzer`.
- Jail call recordings — review for any defendant statements bearing on intent or self-defense, and for State-witness contamination. Route to `dw-jail-call-analyzer`.
- Social media records (subpoena where appropriate). Route to `dw-social-media-auditor`.
...
All deliverables produced by this skill are internal work product unless the attorney directs otherwise. Apply work-product marking per `dw-shared-protocols/references/attorney-work-product-marking.md`. Output paths anchor on the Cowork Analysis formula:
...
2. Section 1 — Case summary. Charges, facts, posture, jurisdictional notes (state-only here; if federal exposure exists, route also to `dw-firearms-specialist`).
...
- **Habitual-Offender Predicate Audit** — generated jointly with `dw-habitual-offender-auditor`.
...
- `dw-pretrial-motion-library` — the motion-and-discovery checklist (Module J) seeds template selection.
- `dw-jury-instructions-builder` — verdict-form architecture, pattern-charge proposals, no-duty-to-retreat language, manslaughter-conversion charges.
- `dw-sentencing-mitigation-specialist` — death-eligibility mitigation, *Miller* mitigation, Art. 893 analysis where available, *Dorthey* excessive-sentence record.
- `dw-habitual-offender-auditor` — predicate audit, no-bill negotiation strategy.
- `dw-cross-exam-architect` — element-by-element cross-exam outlines for State witnesses, medical examiner, ballistics expert.

### This skill PAIRS WITH:

- `dw-firearms-specialist` — when a firearm is the instrumentality of the violent crime (most armed-robbery and many homicide cases). The 14:64.3 enhancement is litigated jointly.
- `dw-witness-threat-matrix` — domestic-violence and gang-related cases where witness intimidation, recantation, and family pressure shape the trial posture.

### This skill READS FROM:

- `dw-case-brain` — structured case context.
- `dw-discovery-orchestrator` and `dw-discovery-compliance-monitor` — discovery tracking and deficiency detection.
- `dw-eyewitness-identification-auditor` — ID reliability evaluation.
- `dw-confession-interrogation-auditor` — *Miranda* / voluntariness audit.
- `dw-crime-scene-auditor` — scene reconstruction.
- `dw-mobile-forensic-auditor` and `dw-forensic-dump-analyzer` — phone evidence.
- `dw-jail-call-analyzer` — recorded calls.
- `dw-brady-giglio-auditor` — impeachment file.
- `dw-404b-opposition` — Prieur opposition work.

### This skill ROUTES TO (after case analysis):

- `dw-suppression-motion` — for any 4A challenge.
- `dw-bond-and-release-motion` — for bond reduction.
- `dw-plea-negotiation-analyzer` — for plea-vs.-trial modeling using the sentencing matrix.
- `dw-trial-notebook-builder` — for trial-day assembly.
- `dw-trial-day-assistant` — for in-trial decision support.
- `dw-appellate-error-monitor` — for record preservation tracking.
- `dw-post-conviction-relief` — if conviction returns and PCR claims (IAC under *Strickland v. Washington*, 466 U.S. 668 (1984); *Miller / Montgomery* retroactivity) are in play.
...
7. **Always check for federal adoption.** Where a violent crime has a firearms component, route in parallel to `dw-firearms-specialist` to assess § 924(c) and § 922(g) exposure. Do not assume the case stays in state court.
8. **Always run the habitual-offender predicate audit.** Habitual exposure can transform every line in the sentencing matrix. Build the spreadsheet in Module I early; route to `dw-habitual-offender-auditor`.
...
- [ ] Module I habitual-offender predicate audit started; routed to `dw-habitual-offender-auditor`.
- [ ] Module J motion and discovery package routed to `dw-pretrial-motion-library`.
