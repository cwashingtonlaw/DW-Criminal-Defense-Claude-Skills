...
*Reflects `dw-criminal-defense` v5.9 (Barone Discovery Workflow Audit) and the full skill suite as of May 2026. Supersedes v1.0 (March 2026).*
...
Every criminal defense case at D&W progresses through three phases (`dw-criminal-defense` v5.9). This is the firm's single source of truth for case processing. Never skip phases or advance past a quality gate without confirmation.
...
|---|---|---|---|
| 1 | Report 0 — Neutral Inventory | `dw-neutral-inventory` | Theory-neutral catalog of all discovery (6 modules: docs, media, physical, witnesses, completeness flags, verification status) |
| 2 | Report 1 — Timeline (with Certainty) | `dw-timeline-builder` | Chronological events with CONFIRMED/PROBABLE/DISPUTED/UNCONFIRMED/ALLEGED ratings |
| 3 | Report 2 — Prosecution's Case Summary | `dw-criminal-defense` Phase 2 | State's theory, elements, evidence, timeline |
| 4 | Report 2a — Theory Deconstruction | `dw-theory-deconstructor` | Decomposes state's theory into facts / inferences / assumptions; Gap Analysis Matrix; Alternative Inference Table |
| 5 | Report 3 — Immediate Red Flags | `dw-criminal-defense` Phase 2 | Constitutional issues, evidentiary gaps, procedural defects |
| 6 | Report 4 — Competing Defense Theories | `dw-criminal-defense` Phase 2 | **Multiple** viable defense narratives, each with strength/weakness/viability assessment |
| 7 | Report 4a — Theory Selection Memo | `dw-criminal-defense` Step 2A | Attorney-driven: selected theory, rationale, key evidence, vulnerabilities, pivot triggers |
| 8 | Theory-to-Workplan | `dw-theory-to-workplan` | 7-stream action plan (Investigation, Discovery, Experts, Motions, Witnesses, Exhibits, Narrative) |
| 9 | Adversarial Stress Test | `dw-adversarial-stress-test` | Prosecutor red-team simulation with defense counter-responses, jury perception risk |

**Important behavioral rule:** Reports 4a, 8, and 9 require the attorney to have selected a theory. Never run `dw-theory-to-workplan` or `dw-adversarial-stress-test` until Report 4a exists with attorney sign-off.
...
| "New case," "case intake," "run Phase 1/2/3" | `dw-criminal-defense` |
| "Intake," "new client meeting," "first meeting" | `dw-client-intake-interview` |
| "What skills do we have," "which skill handles X" | `dw-skill-index` |
| "Build the trial notebook" | `dw-trial-notebook-builder` |
| "New discovery arrived" | `dw-discovery-orchestrator` |
| "Transcribe the evidence" | `dw-transcript-router` |

### Barone Workflow Skills (NEW)
| Trigger | Skill |
|---------|-------|
| "Neutral inventory," "catalog the evidence," "Report 0," "what do we have" | `dw-neutral-inventory` |
| "Deconstruct the theory," "facts vs inferences," "Report 2a," "assumption audit" | `dw-theory-deconstructor` |
| "Build a workplan," "theory to workplan," "task list for trial" | `dw-theory-to-workplan` |
| "Stress test," "red team," "prosecutor's perspective" | `dw-adversarial-stress-test` |

### Evidence Auditing
| Evidence Type | Skill |
|---------------|-------|
| Phone extraction methodology (HOW it was dumped) | `dw-mobile-forensic-auditor` |
| Phone dump content analysis (WHAT's on it) | `dw-forensic-dump-analyzer` |
...
| **DNA / forensic biology** (STR, mixtures, STRmix, TrueAllele, IGG, mtDNA, Y-STR) | `dw-dna-forensic-biology-auditor` |
| **Crime lab** (drug ID, toxicology, BAC, R.S. 15:499 certificate challenges) | `dw-crime-lab-auditor` |
| Witness statement inconsistency audit | `dw-witness-statement-analyzer` |
| Habitual offender bill audit | `dw-habitual-offender-auditor` |
| SQLite / WAL file recovery | `dw-sqlite-recovery` |
| **Jail calls** (Securus/GTL/ViaPath/NCIC/IC Solutions) | `dw-jail-call-analyzer` |
...
| Sex offenses (includes SANE-exam audit, rape shield) | `dw-sex-offense-specialist` |
| Firearms offenses (state and federal) | `dw-firearms-specialist` |
| **Violent crimes** (homicide, manslaughter, agg battery, armed robbery, kidnapping, home invasion, self-defense) | `dw-violent-crime-specialist` |

### Motions & Pleadings
| Motion Type | Skill |
|-------------|-------|
| Suppression (4th/5th/6th Amendment) | `dw-suppression-motion` |
...
| Task | Skill |
|------|-------|
| Cross-examination outlines (state witnesses) | `dw-cross-exam-architect` |
| Direct-examination outlines (defense witnesses) | `dw-direct-exam-architect` |
| **Opening + Closing + Theme Tracker + Rebuttal Memo** | `dw-trial-narrative-builder` |
...
| Witness threat ranking (post-jail-call cross-feed) | `dw-witness-threat-matrix` |
| Mock juror reaction modeling | `dw-jury-focus-group` |
| Exhibit list / authentication tracker | `dw-exhibit-manager` |
| **Real-time trial-day support** (objection log, witness scorecards, juror obs) | `dw-trial-day-assistant` |
| Issue-code based docket tracking | `dw-issue-code-tracker` |
| Court date / jail status tracker | `dw-court-jail-tracker` |

### Appellate & Post-Conviction
| Task | Skill |
|------|-------|
| Error preservation during trial | `dw-appellate-error-monitor` |
| Direct-appeal brief assembly | `dw-appellate-brief-builder` |
| PCR / federal habeas / sentence modification | `dw-post-conviction-relief` |
| Outcome documentation | `dw-case-disposition` |

### Discovery & Compliance
| Task | Skill |
|------|-------|
| Discovery compliance tracking (with **Barone 7-bucket classification**) | `dw-discovery-compliance-monitor` |
...
| Image filename Bate stamping | `dw-image-filename-stamp` |
...
| Cross-case DMAR synthesis | `dw-dmar-synthesizer` |

Use `dw-transcript-router` as the single entry point — it handles parish routing automatically. Both pipelines produce a standardized **Defense Media Analysis Report** (.docx) including the **6-category Report-vs-Recording Matrix** (Narrative Match / Omissions / Additions / Timing / Quote Accuracy / Procedural Compliance).

### Client Communication & Operations
| Task | Skill |
|------|-------|
| Standard client letters, updates, document requests | `dw-client-communication-drafter` |
| Time-entry narrative drafting | `dw-billing-narrative-generator` |
| Statute and case-law lookup | `dw-case-law-researcher` |

### Shared References (Read by Other Skills — Don't Invoke Directly)
| Skill | What it provides |
|-------|------------------|
| `dw-shared-protocols` | Work-product marking, output-path formula, **verification protocol**, citation standards, template selection |
| `dw-data-contracts` (v1.2) | Output schemas: DMAR (with 6-category matrix), auditor reports, cross/direct outlines, Case Tables, Case Brain entries, discovery ledger (with bucket column) |

### Retired Skills (Do NOT Reference)
- `dw-lwop-populator` — **retired in v5.3**, functionality merged into `dw-criminal-defense` Phase 1 Step 3
- `dw-template-selector` — **retired in May 2026**, template selection protocol consolidated into `dw-shared-protocols/references/`
...
2. Present ranked results to the attorney using the template selection protocol at `dw-shared-protocols/references/`.
...
Add a Verification Summary at the end of every deliverable using this protocol. See `dw-shared-protocols/references/verification-protocol.md`.
...
Maintain all existing color coding, dropdown lists, and formatting in every sheet. Color specs are in `dw-criminal-defense/references/color-coding.md`.
...
Generated by `dw-criminal-defense` Phase 2 Step 2 with the Barone pre/post-analysis around them:
...
| 3 | Immediate Red Flags | **HIGH ★** | Routes to `dw-suppression-motion` / `dw-expert-witness-evaluator` |
| 4 | **Competing Defense Theories** (revised v5.9) | Standard | Attorney selects → Report 4a |
| 4a | **Theory Selection Memo** (new v5.9) | **HIGH ★** | Routes to `dw-adversarial-stress-test` + `dw-theory-to-workplan` |
| 5 | Viable Legal Defenses | Standard | Routes to `dw-404b-opposition`, `dw-sentencing-mitigation-specialist`, `dw-habitual-offender-auditor` |
| 6 | Memorable Theme | Standard | Feeds `dw-trial-narrative-builder` |
| 7 | Table of Missing Discovery | **HIGH ★ → Auto-Action** | Auto-drafts Missing Discovery Demand Letter; routes to `dw-brady-giglio-auditor` |
| 8 | Key Witness Impeachment Plan | **HIGH ★ → Auto-Action** | Auto-generates Impeachment Worksheets; routes to `dw-cross-exam-architect` |
...
2. **Chain of Custody Audit** → route to `dw-chain-of-custody-auditor`
3. **Specialist Evidence Routing** → dispatch to evidence-type auditors (`dw-eyewitness-identification-auditor`, `dw-confession-interrogation-auditor`, `dw-mobile-forensic-auditor`, `dw-video-evidence-auditor`, `dw-cell-site-geolocation-auditor`, `dw-social-media-auditor`, `dw-child-forensic-interview-auditor`, `dw-expert-witness-evaluator`, `dw-jail-call-analyzer`)
4. **Charge-Type Specialist Routing** → dispatch to the relevant specialist (`dw-drug-offense-specialist`, `dw-dwi-specialist`, `dw-sex-offense-specialist`, `dw-firearms-specialist`, `dw-violent-crime-specialist`)
5. **Barone Pre-Analysis (Step 1E)** → `dw-neutral-inventory` (Report 0) + `dw-theory-deconstructor` (Report 2a)
...
11. **Barone Workflow gates:** Never run `dw-theory-to-workplan` or `dw-adversarial-stress-test` until Report 4a (Theory Selection Memo) exists with attorney sign-off.
...
2. **If no Case Brain exists** → create one → invoke `dw-client-intake-interview` if pre-case-file, or `dw-criminal-defense` Phase 1 if case file exists
...
When the attorney is in trial and says "log this objection," "today's witness," or "trial day," route to **`dw-trial-day-assistant`**. That skill produces short, scannable outputs designed for use during breaks and at counsel table:

- Daily docket
- Real-time objection log (auto-feeds `dw-appellate-error-monitor`)
- Witness scorecards (auto-feeds `dw-cross-exam-architect` for next-day prep)
- Exhibit tracker
...
Final polish rolls into the Trial Notebook via Phase 3 Step 12 (`dw-trial-notebook-builder`).
...
*These project instructions reflect Daniels & Washington criminal defense workflows as of May 2026 (`dw-criminal-defense` v5.9 — Barone Discovery Workflow Audit). They are designed to be placed in the Custom Instructions field of any Cowork project containing a criminal defense case file. Update these instructions whenever the master workflow or the skill ecosystem is revised.*
