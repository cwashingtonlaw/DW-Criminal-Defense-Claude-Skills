...
- **Migration note.** The `assets/CASE PROFILE.docx` template predates v5.11. On first generation, add the § 1 Prosecution's Theory prose block and the two new Seized Property columns (Evidence ID / PR#, Owner Basis) per the Generation Procedure, and flag it in the completion notes. Build hygiene: write to a fresh temp filename, validate, then delete-and-replace the live file (and close it in Word first) to avoid the stale-cache revert.
